...
public class CsvDataSource implements DataSource {
  ...
  public boolean hasLatLonColumns(Table table) {
    Set<String> lowerColNames =
        table.columnNames().stream().map(String::toLowerCase).collect(Collectors.toSet());
  ...
  public TabularResponse fetchTabularData(TabularRequest tabularRequest) {
    ...
    if (Files.isDirectory(path)) {
      ...
      for (Table table : tables) {
        ...
        if (columns.isEmpty()) {
          columns.addAll(
              resultsTable.columns().stream()
                  .map(this::getTabularColumnFromTableSawColumn)
                  .toList());
        }
      }
  ...
  public MetadataResponseV1 getFileMetadata(MetadataRequest metadataRequest) {
    ...
    metadataResponse.setOriginalColumns(
        table.columns().stream().map(this::getTabularColumnFromTableSawColumn).toList());
  ...
  public Table readCsvFromFile(Path filePath) {
    ...
    try (InputStream stream = Files.newInputStream(filePath)) {
      CsvReadOptions options = CsvReadOptions.builder(stream).header(true).sample(true).build();
      return Table.read().usingOptions(options).setName(fileName);

    } catch (Exception firstException) {
      LOG.warn(
          "Initial type inference failed for '{}'. Retrying with full parsing (sample=false)",
          fileName);

      // Attempt 2: fallback to more conservative parsing
      try (InputStream retryStream = Files.newInputStream(filePath)) {
        CsvReadOptions fallbackOptions =
            CsvReadOptions.builder(retryStream).header(true).sample(false).build();
                              ...
                              columnName -> {
                                ...
                                String propertyVal =
                                    colType == ColumnType.STRING
                                            || colType == ColumnType.LOCAL_DATE_TIME
  ...
  private Map<String, List<?>> getUniqueValuesForColumns(
      Table table, List<TabularColumn> tabularColumns) {
