...
/**
 * Caches parsed evaluation Tables per CSV path. The cache must live on a
 * separate bean (not in {@link ModelEvaluationService}) because Spring's
 * proxy-based @Cacheable only intercepts cross-bean calls — self-invocation
 * within the same class bypasses the cache.
 */
@Service
