...
/**
 * ExtremeXP implementation of the ExperimentService. Connects to ExtremeXP execution engine to
...
public class ExtremeXPExperimentService implements ExperimentService {
  ...
  public ResponseEntity<List<Experiment>> getExperiments(
    ...
    try {
      ...
      if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
        List<Map<String, Object>> experimentsList = (List<Map<String, Object>>) response.getBody();

        List<Experiment> experiments =
            experimentsList.stream()
                .filter(m -> m != null && !m.isEmpty())
                .map(
                    expMap ->
                        (Map<String, Object>)
                            expMap.values().iterator().next()) // Extract inner experiment object
        ...
        for (Experiment experiment : experiments) {
          ...
          HttpEntity<Map<String, Object>> entityForMetrics =
              new HttpEntity<>(requestBody, headersInitializer());

          // Second API call to fetch metrics data
          ResponseEntity<List> responseMetrics =
              restTemplate.exchange(requestUrlMetc, HttpMethod.POST, entityForMetrics, List.class);

          if (responseMetrics.getStatusCode() == HttpStatus.OK
              && responseMetrics.getBody() != null) {
            List<Map<String, Object>> metricsResponse =
                (List<Map<String, Object>>) responseMetrics.getBody();
            ...
            System.out.println(
                "Failed to retrieve metrics for experiment "
                    + experiment.getId()
                    + ". Status Code: "
                    + responseMetrics.getStatusCode());
  ...
  @Override
  @Cacheable(
      value = "experimentCache",
      key = "#experimentId",
      unless = "#result.body == null or #result.body.status != 'completed'")
    ...
    try {
      ...
      if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
        Map<String, Object> experimentData =
            (Map<String, Object>) response.getBody().get("experiment");
        ...
        if (responseMetrics.getStatusCode() == HttpStatus.OK && responseMetrics.getBody() != null) {
          ...
          System.out.println(
              "Failed to retrieve metrics. Status Code: " + responseMetrics.getStatusCode());
          System.out.println("Metrics Response Body: " + responseMetrics.getBody());
        }
  ...
  @Cacheable(
      value = "runsCache",
      key = "'runsfor::' + #experimentId",
      unless = "#result.body.?[status.name() != 'COMPLETED'].size() > 0")
  public ResponseEntity<List<Run>> getRunsForExperiment(String experimentId) {
    ...
    try {
      ...
      if (response.getStatusCode() == HttpStatus.OK
        ...
        List<Run> runs =
            responseObjects.parallelStream()
  ...
  @Cacheable(
      value = "runsCache",
      key = "'runfor::' + #experimentId + '::' + #runId",
      unless = "#result.body.getStatus().name() != 'COMPLETED'")
  public ResponseEntity<Run> getRunById(String experimentId, String runId) {
    try {
      HttpEntity<String> entity = new HttpEntity<>(headersInitializer());
      String workflowUrl = workflowsApiUrl + "/workflows/" + runId;
      ResponseEntity<Map> workflowResponse =
          restTemplate.exchange(workflowUrl, HttpMethod.GET, entity, Map.class);

      // Extract workflow data from the response and prepare the Run object
      Map<String, Object> workflowData =
          (Map<String, Object>) workflowResponse.getBody().get("workflow");
  ...
  public ResponseEntity<UserEvaluationResponse> submitUserEvaluation(
    ...
    try {
      ...
      ResponseEntity<List> queryResponse =
          restTemplate.exchange(queryUrl, HttpMethod.POST, queryEntity, List.class);
      ...
      if (responseList == null || responseList.isEmpty()) {
        ...
        HttpEntity<Map<String, Object>> createEntity =
            new HttpEntity<>(createBody, headersInitializer());
        ...
        if (createResponse.getStatusCode() == HttpStatus.CREATED
          ...
          return ResponseEntity.status(HttpStatus.CREATED)
              .body(
                  new UserEvaluationResponse("success", "User evaluation submitted successfully"));
        } else {
        ...
        HttpEntity<Map<String, Object>> updateEntity =
            new HttpEntity<>(updateBody, headersInitializer());

        ResponseEntity<String> updateResponse =
            restTemplate.exchange(updateUrl, HttpMethod.POST, updateEntity, String.class);

        if (updateResponse.getStatusCode() == HttpStatus.OK) {
          return ResponseEntity.ok(
              new UserEvaluationResponse("success", "User evaluation updated successfully"));
        } else {
  ...
  public ResponseEntity<ControlResponse> controlLifeCycle(ControlRequest controlRequest) {
    ...
    String entityId =
        (controlRequest.getExperimentId() != null)
            ? controlRequest.getExperimentId()
  ...
  private Run runPreparation(Map<String, Object> responseObject) {
    ...
    run.setExperimentId(
        responseObject.get("experimentId") != null
            ? responseObject.get("experimentId").toString()
            : null);
    run.setStartTime(
        responseObject.get("start") != null
            ? parseIsoDateToMillis(responseObject.get("start").toString())
            : null);
    run.setEndTime(
        responseObject.containsKey("end") && responseObject.get("end") != null
            ? parseIsoDateToMillis(responseObject.get("end").toString())
            : null);

    // Status handling
    String statusStr =
        responseObject.get("status") != null ? responseObject.get("status").toString() : null;
    try {
    ...
    if (responseObject.containsKey("metadata")) {
      ...
      for (Map.Entry<String, Object> entry : metadata.entrySet()) {
        if (entry.getValue() instanceof String) {
          ...
          tags.put(
              entry.getKey(), entry.getValue().toString()); // Convert non-string values to String
        }
      }
    }
    ...
    List<Map<String, Object>> metricsList =
        (List<Map<String, Object>>) responseObject.get("metrics");
      ...
      for (Map<String, Object> taskObj : tasksList) {
        ...
        if (taskObj.containsKey("parameters")) {
          List<Map<String, Object>> parameters =
              (List<Map<String, Object>>) taskObj.get("parameters");
        ...
        Task task =
            new Task(taskName, taskType, variant, taskStartTime, taskEndTime, taskTags, taskId);
        tasks.add(task);
    ...
    if (taskObj.containsKey(key)) {
      ...
      for (Map<String, Object> dataset : datasetList) {
        ...
        if (datasetList.stream()
                .filter(d -> d.containsKey("name") && d.get("name").equals(dataset.get("name")))
                .count()
            > 1) {
      ...
      if (dataset.containsKey("metadata")) {
        ...
        if (metadata.containsKey("file_name")) {
          ...
          throw new IllegalArgumentException(
              "Metadata does not contain file_name for HTTP dataset");
        }
      } else {
  ...
  public ResponseEntity<List<Run>> reorderWorkflows(ReorderRequest reorderRequest) {
    String requestUrl =
        experimentationEngineApiUrl
            + "/experiments-sort-workflows/"
  ...
  public ResponseEntity<CreateRunResponse> createRun(CreateRunRequest request) {
    throw new UnsupportedOperationException(
        "This operation has not been implemented yet for ExtremeXp.");
  }
}
