...
public class FileHelper {
  ...
  private HttpRequest.Builder requestBuilder(String path) {
    ...
    return HttpRequest.newBuilder().uri(URI.create(zenohBaseUrl + "/file/" + path));
  }

  @Async
  public CompletableFuture<InputStream> downloadFromHTTP(
    ...
    HttpRequest request =
        requestBuilder(dataSource.getSource()).GET().header("Authorization", authorization).build();
