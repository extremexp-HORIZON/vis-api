...
/**
 * REST controller that provides experiment tracking endpoints. This controller delegates requests
 * to the appropriate execution engine implementation (e.g., MLflow, ExtremeXP). It provides
 * endpoints to retrieve experiments, fetch specific experiment details, list associated runs,
 * retrieve specific run details, and fetch recorded metric values.
 */
@RestController
...
public class ExperimentController {
  ...
  /**
   * Retrieves a paginated list of experiments. Supports pagination with `limit` and `offset`
   * parameters.
   *
   * @param limit The maximum number of experiments to return (default: 10, max: 100).
   * @param offset The starting index for retrieval (default: 0).
   * @return A ResponseEntity containing a list of experiments and their total count.
   */
  @GetMapping
   ...
   *
   * @param experimentId The unique identifier of the experiment.
   * @return A ResponseEntity containing a list of runs associated with the experiment.
   */
   ...
   *
   * <p>The response includes metadata, parameters, and metrics for the run.
   *
  ...
  public ResponseEntity<Run> getRunById(
      @PathVariable String experimentId, @PathVariable String runId) {
   ...
   *
   * <p>Returns a list of recorded metric values for the requested metric.
   *
   * @param experimentId The ID of the experiment the run belongs to.
   * @param runId The unique identifier of the run.
  ...
  public ResponseEntity<List<Metric>> getMetricValues(
    ...
    return experimentServiceFactory
        .getActiveService()
        .getMetricValues(experimentId, runId, metricName);
  }
  ...
  public ResponseEntity<List<Metric>> getAllMetrics(
    ...
    return experimentServiceFactory
        .getActiveService()
        .getAllMetrics(experimentId, runId, metricName);
  }
  ...
  public ResponseEntity<Map<String, List<Metric>>> getAllMetricsBatch(
    ...
    return experimentServiceFactory
        .getActiveService()
        .getAllMetricsBatch(experimentId, runId, metricNames);
  }

  /**
   * This endpoint abstracts user feedback submitted via the dashboard. Most experiment tracking
   * tools do not natively support such evaluations, so implementations may store them using
   * available mechanisms — such as run tags, metadata, or annotations. This abstraction provides a
   * unified way to capture user input, while allowing backend flexibility based on each tool’s
   * capabilities.
   *
   * <p>Returns a list of recorded metric values for the requested metric.
   *
  ...
  public ResponseEntity<UserEvaluationResponse> submitUserEvaluation(
    ...
    return experimentServiceFactory
        .getActiveService()
        .submitUserEvaluation(experimentId, runId, userEvaluation);
  }

  /**
   * Controls the experiment's lifecycle through state transitions. This endpoint allows managing
   * the experiment workflow by submitting control requests that trigger state changes (e.g., start,
   * pause, resume, stop).
   *
   * <p>The implementation handles the transition logic according to the experiment engine's
   * capabilities.
   *
   * @param controlRequest The control request containing the desired action and target
  ...
  public ResponseEntity<ControlResponse> getExperimentControl(
      @RequestBody ControlRequest controlRequest) {
    return experimentServiceFactory.getActiveService().controlLifeCycle(controlRequest);
  }

  /**
   * Reorders workflows based on the provided reorder request. This endpoint allows rearranging the
   * execution order of workflows according to the specified criteria in the reorder request.
   *
   * <p>The implementation handles the reordering logic according to the experiment engine's
   * capabilities and returns the updated list of runs.
   *
   * @param reorderRequest The reorder request containing the criteria and parameters for reordering
   *     workflows
  ...
  public ResponseEntity<CreateRunResponse> createRun(
      @PathVariable String experimentId, @RequestBody CreateRunRequest request) {
